...
export class Rechnung extends Command {
    ...
    async execute(interaction: CommandInteraction): Promise<void> {
        ...
        if (options.getSubcommand() === 'suchen') {
            try {
                ...
                let rechnungen = rows as IBilling[]
                    ...
                    for (let i = limit * (page - 1); i < rechnungen.length; i++) {
                        ...
                        fields.push({
                            ...
                            value: `Empfänger: ${rechnungen[i].receiver_name} (\`${
                                ...
                                rechnungen[i].limit_pay_date ?? 'Kein Limit'
                            }\nGebühren: ${rechnungen[i].fees_amount}\nBezahlt am: ${
                                rechnungen[i].paid_date ?? 'Nicht bezahlt'
                            }`,
                            inline: false,
                        })
                    }
        ...
        } else if (options.getSubcommand() === 'anzeigen') {
            try {
        ...
        } else if (options.getSubcommand() === 'bezahlen') {
            try {
                let rechnungsnummer = options.getInteger('id') as number
                let [rows] = await Database.query(
                    'SELECT * FROM immobilling WHERE id = ? LIMIT 1',
                ...
                if (rechnungen[0].status != 'unpaid') {
                    ...
                    const channel = (await interaction.guild?.channels.fetch(
                        Config.Discord.Channel.S1_IMMO_BILLING,
                    )) as TextChannel
        ...
        } else if (options.getSubcommand() === 'löschen') {
            try {
                let rechnungsnummer = options.getInteger('id') as number
                let [rows] = await Database.query(
                    'SELECT * FROM immobilling WHERE id = ? LIMIT 1',
        ...
        } else if (options.getSubcommand() === 'erstellen') {
            try {
                let steamid = options.getString('steamid') ?? ''
                if (!steamid.startsWith('steam:')) steamid = 'steam:' + steamid
                ...
                if (sender != 'dortmund') {
                    const senderquery = await Database.query<RowDataPacket[]>(
                        'SELECT * FROM jobs WHERE name = ?',
                        [sender],
                    )
                    if (senderquery[0].length === 0) {
                        sender = 'dortmund'
                        sendername = 'Stadt Dortmund'
                    } else {
                        let { name, label } = senderquery[0][0] as {
                            name: string
                            label: string
                        }
                        sender = name
                ...
                let limit_pay_date_string = limit_pay_date
                    .toISOString()
                    .slice(0, 19)
                    .replace('T', ' ')
